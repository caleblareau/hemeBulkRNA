...
metaData <- read.table("lookupGEO.txt", header = TRUE, stringsAsFactors = FALSE)
...
shortNames <- gsub('.{17}$', '', list.files("counts", full.names = FALSE))
fullnames <- list.files("counts", full.names = TRUE)
...
typevec <- metaData$Source
names(typevec) <- metaData$UID
...
autoGenes <- mergedDF1[mergedDF1$chr %in% as.character(1:22), "gene"] %>% as.character()
df <- df[row.names(df) %in% autoGenes, ]
...
doLimmaGetBed <- function(inGroup, prop = 0.1, pad = 100000){
  ...
  groups <- as.numeric(typevec[colnames(df)] == inGroup)
  z <- DGEList(counts = data.matrix(df), group = groups)
  z <- calcNormFactors(z)
  ...
  results <- as.data.frame(topTable(fit, number = round(nrow(df)*prop), sort.by = "t"))
  ...
  write.table(rdf[,c("chrchr", "start", "end", "V2")], paste0("bedfiles/", inGroup, ".28Aug2017.bed"), 
                    sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
  inGroup
}

lapply(unique(typevec), doLimmaGetBed)
